...
export function simulateSolarGeneration(
    ...
    for (let i = 0; i < totalIntervals; i++) {
        const timeDt = DateTime.fromJSDate(startDate).startOf('day').plus({ minutes: i * 15 });
        const time = timeDt.toJSDate();

        let cloudCover = 0;
        if (weatherData) {
            const hourKey = timeDt.toFormat("yyyy-MM-dd'T'HH:00");
            if (weatherData[hourKey] !== undefined) {
                cloudCover = weatherData[hourKey] / 100;
            }
        }
